...
imgElement.alt = ("A beautiful rainbow");
...
attributes.
*/
function createColorBox(color, num){
...
function renderPaletteRow(arr, parent){
  ...
  for(let colorObj of arr){
    let tempBox = createColorBox(colorObj, (getElementWidth(parent))/arr.length);
    newDivElement.appendChild(tempBox);
  }
...
function renderPaletteTable(){
  ...
  for(let obj in COLORS_9){
    renderPaletteRow(COLORS_9[obj], mainElement);
  }
